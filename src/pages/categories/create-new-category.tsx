...
import { createCategory } from "Redux/Action/Categories/category.action";
...
const NewCategory: NextPage = () => {
    //Selector
    const { success, message, loading } = useAppSelector(state => state.createCategory);
    ...
    const onSubmit: SubmitHandler<Inputs> = (data, e) => {
        dispatch(createCategory(data, images[0]?.file as File))
    }
    //Effect
    useEffect(() => {
    ...
    return (
        <Layout title="Create new category | Changing Experience!" active="category">
            {profile.role === "editor" ? (
