...
import { createBrand } from "Redux/Action/Brands/brand.action";
...
const CreateBrands: NextPage = () => {
    //Selector
    const { profile } = useAppSelector(state => state.profile);
    const { message, success, loading } = useAppSelector(state => state.createBrand);
    //State
    ...
    const onSubmit: SubmitHandler<Inputs> = (data, e) => {
        dispatch(createBrand(data, images[0]?.file as File))
    }
    //Effect
    useEffect(() => {
    ...
    return (
        <Layout title="Create new brand | Changing Experience!" active="brand">
            {profile.role === "editor" ? (
