...
import { updateDynamicBanners } from "Redux/Action/Home/home.action";
...
const Update = () => {
    ...
    const { dynamics } = useAppSelector(state => state.singleDynamicBanner);
    const { message, loading, success } = useAppSelector(state => state.updateDynamicBanner);
    ...
    const {
        ...
        defaultValues: {
            ...
            bannerType: BannerTypeOptions.find((item) => item.value === dynamics?.bannerType),
            totalNumber: TotalNumberOptions.find((item) => item.value === dynamics?.totalNumber),
            publish: dynamics?.publish,
            section: { label: dynamics?.section?.name, value: dynamics?.section?.id },
            banners: dynamics?.banners
        }
    });
    ...
    const onSubmit: SubmitHandler<Inputs> = (value) => {
        ...
        dispatch(updateDynamicBanners(BannerData, dynamics?.id))
    }
                                        ...
                                        {({
                                            ...
                                            <Box onClick={onImageUpload} sx={{ cursor: "pointer" }}>
                                                <AspectRatio ratio="4/3" style={{ width: "100%" }} >
                                                    <>
                                                        {imageList.length === 0 && !dynamics?.banners[i]?.url &&
                                                        ...
                                                        {imageList.length === 0 && dynamics?.banners[i]?.url &&
                                                            <Image src={process.env.NEXT_PUBLIC_IMAGE_PATH + dynamics.banners[i].url} alt="banner" key={i} layout={'fill'} />
                                                        }
                                                    </>
                                                </AspectRatio>
                                            </Box>
